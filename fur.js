...
export function fur(surfaceMesh, hairs = 1000) {
    ...
    const hairGeometry = new THREE.ConeGeometry(.1, hairLength, 2);
    hairGeometry.translate(0, hairLength / 2, 0);
    hairGeometry.rotateX(-Math.PI / 2);
    ...
    const mesh = new InstancedUniformsMesh(hairGeometry, hairMaterial, hairs);
    mesh.position.copy(surfaceMesh.position);
    mesh.rotation.copy(surfaceMesh.rotation);
    mesh.scale.copy(surfaceMesh.scale);
    ...
    // const phiRange = Math.PI / 4
    // const thetaRange = Math.PI / 6
    for (let i = 0; i < hairs; i++) {
        sampler.sample(position, normal);
        matrix.makeTranslation(position.x, position.y, position.z);

        // const noisePos1 = position.clone().multiplyScalar(.2);
        // const noisePos2 = noisePos1.clone().addScalar(1000);
        // const phi = perlin.get3(noisePos1) * (phiRange * 2) - phiRange;
        // const theta = perlin.get3(noisePos2) * (thetaRange * 2) - thetaRange;
        // const rotation = new THREE.Euler(phi, 0, theta);
        // normal.applyEuler(rotation);
        matrix.lookAt(new THREE.Vector3(), normal, new THREE.Vector3(0, 1, 0));
        mesh.setMatrixAt(i, matrix);
        mesh.setUniformAt('normal', i, normal)


        const clrPos = position.multiply(surfaceMesh.scale).add(surfaceMesh.position)
        const clr = getColor(clrPos)
        mesh.setColorAt(i, clr)
        mesh.setUniformAt('color', i, new THREE.Vector3(clr.r, clr.g, clr.b))
    }
...
export const furBaseMaterial = new THREE.MeshStandardMaterial({
    vertexColors: true,
    clippingPlanes: [
        new THREE.Plane(new THREE.Vector3(0, 1, 0), 0),
    ],
